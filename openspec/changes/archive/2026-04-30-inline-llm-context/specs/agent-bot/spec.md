...
It SHALL then call `backend.complete_step(messages, self.tools)` repeatedly. On each `ToolUse` result it SHALL invoke the matched tool's `fn`, append the assistant's tool-use message and the `tool`-role result to the running context using `[*messages, ...]`, then call `complete_step` again. The loop SHALL terminate when `complete_step` returns a `TextResponse`, and `on_message` SHALL return a `ChatMessage` with that text.
