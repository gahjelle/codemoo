...
Defines RetryBot (Undo) — the tenth bot in the demo progression. RetryBot extends MemoryBot with a per-turn retry counter that detects when the LLM is stuck calling the same tool repeatedly, escalates after three identical calls, and surfaces a structured failure summary so the user can intervene with guidance.
