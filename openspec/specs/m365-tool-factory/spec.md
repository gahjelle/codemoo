...
TBD — defines `make_graph_tools()`, a factory function that constructs Microsoft Graph `ToolDef` instances as closures over an `M365Config`. Graph tools are not registered statically in `TOOL_REGISTRY`; they are built at startup and injected into `make_bots(extra_tools=...)`.
