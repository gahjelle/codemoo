# Spec: whoami-demo-game

## Purpose

Defines `demo/whoami.py` — a daily-seeded famous-person guessing game used as the pre-seeded failure artifact for the RetryBot demo. The script contains a deliberate API key typo (`MISTAKE_API_KEY`) that causes a consistent `KeyError` at runtime, which RetryBot surfaces after three retries.
