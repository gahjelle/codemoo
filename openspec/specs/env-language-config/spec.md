...
TBD — defines the `CODEMOO_LANGUAGE` environment variable and the `language_instruction()` helper in `codemoo.config` that injects a language instruction into infrastructure LLM prompts.
