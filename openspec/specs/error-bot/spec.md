...
Defines the ErrorBot participant, which is always present in every chat session and surfaces exceptions from other participants as styled in-chat error bubbles. ErrorBot uses an LLM to generate personality-flavored error messages, with a plain-text fallback if the LLM is unavailable.
