...
`ToolBot` SHALL include a default `instructions: str` field (re-declared in `ToolBot` with `_INSTRUCTIONS` as default) that informs the LLM that tools are available and encourages their use when relevant, without enforcing a rigid coding-only persona.
