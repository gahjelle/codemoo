# Spec: tool-approval-flag

## Purpose

TBD — Defines `format_tool_call()`, a shared utility for formatting tool call signatures with optional per-value truncation, used by `CommentatorBot` and `ApprovalModal`.
