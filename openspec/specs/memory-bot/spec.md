...
TBD — defines MemoryBot, a bot that loads persistent memory from a file at startup and injects it alongside project context into every system prompt, and exposes a `save_memory` tool so the LLM can update that memory.
