# Spec: tool-init-hooks

## Purpose

TBD — defines the `init` hook mechanism on `ToolDef` that allows tools to declare one-time setup (e.g. M365 authentication) which the startup code runs before opening the chat UI.
