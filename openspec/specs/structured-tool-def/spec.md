...
- **THEN** all fields SHALL be accessible as attributes, `requires_approval` SHALL default to `False`, `init` SHALL default to `None`, and `validate` SHALL default to `None`
...
#### Scenario: validate defaults to None
