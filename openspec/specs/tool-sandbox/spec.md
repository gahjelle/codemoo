...
TBD — Defines the sandboxing mechanism that constrains file and shell tools to the session folder, using the `validate` field on `ToolDef` and the `dispatch_tool` helper.
