...
@pytest.mark.asyncio
async def test_successful_command_returns_stdout() -> None:
    result = await run_shell.fn(command="echo hello")
    assert "hello" in result
    assert "exit code: 0" in result


@pytest.mark.asyncio
async def test_failing_command_returns_nonzero_exit_code_without_raising() -> None:
    result = await run_shell.fn(command="exit 1")
    assert "exit code: 1" in result


@pytest.mark.asyncio
async def test_stderr_included_in_output() -> None:
    result = await run_shell.fn(command="echo error >&2; exit 2")
    assert "error" in result
    assert "exit code: 2" in result


@pytest.mark.asyncio
async def test_timeout_returns_message_not_exception() -> None:
    result = await run_shell.fn(command="sleep 10", _timeout=1)
